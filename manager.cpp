        ...
        case 15:
          {
            ...
            int err = pthread_create(&mOnlineThread, &attr, onlineThread, this);
            if(err!=0){
              string tempstr = "can't create online thread: ";
              tempstr.append(strerror(err));
              pDisplay->output(tempstr,CDisplay::ERROR_T);
            }
            pthread_attr_destroy(&attr);
          }
          pDisplay->output("online display started!",CDisplay::NORMAL_T);
        }
      flag = 100;
...
bool CManager::daqCycle()
{
  ...
  size_t buffersize=4096*2+2;//ccusb need buffersize larger than the actual buffer length
...
bool CManager::Config()
{
  ...
  // online data buffer init
  if(fDataBuffer){
    delete fDataBuffer;
  }
  fChannelNum = size*16;
  fDataBuffer = new int[fChannelNum];
  fTriggerID = 0;
  fFlagOdd = false;
  fDataOdd = -1;
  fDecodeStatus.Reset();
...
void CManager::onlineDecode(char* buffer, size_t size)
{
  ...
  // check if previous transfer number is odd/even
  if(fFlagOdd){
    real_size++;
    // add the left char from previous transfer into the buffer
  ...
  // check whether current transfer number is odd/even
  int buffer_length = real_size/2;
  unsigned short* pbuffer = (unsigned short*)buffer;
  if(real_size%2){//odd
    fFlagOdd = true;
    fDataOdd = buffer[real_size];
  }

  // decode implemented (two methods can be implemented)
  for (int i=0; i < buffer_length; i++) {
    // 1) switch implemetation
    switch (fDecodeStatus.fStatus) {
    case E_EventBody: {
      ...
      if(fDecodeStatus.fEventLength == 1){
        if(fDecodeStatus.fChannelIndex != fChannelNum){
          // format error
          // TODO
        }
        else{
          fDecodeStatus.fStatus = E_EventTerminator;
        }
      }
      break;
    }
    case E_EventTerminator: { 
      if (pbuffer[i] != 0xEEEE ) {
        // format error
        //TODO
      }
      else{
        fDecodeStatus.fEventLength = 0xEEEEEEEE;
        fDecodeStatus.fTriggerID_High = 0xEEEEEEEE;
    ...
    case E_EventHeader: {
      if (fDecodeStatus.fEventLength == 0xEEEEEEEE) {
        fDecodeStatus.fEventLength = pbuffer[i];
      }
      else if (fDecodeStatus.fTriggerID_Low == 0xEEEEEEEE){
        fDecodeStatus.fTriggerID_Low = pbuffer[i];
        fDecodeStatus.fEventLength--;
      }
      else{
        fDecodeStatus.fTriggerID_High = pbuffer[i]<<16;
        fTriggerID = fDecodeStatus.fTriggerID_Low + fDecodeStatus.fTriggerID_High;
        fDecodeStatus.fEventLength--;
        if (fDecodeStatus.fEventLength == 1) {
          fDecodeStatus.fStatus = E_EventTerminator;
        }
        else{
          fDecodeStatus.fStatus = E_EventBody;
        }
      }
      fDecodeStatus.fBufferLength--;
      break;
    ...
    case E_BufferHeader: {
      if(fDecodeStatus.fEventNumber == 0xEEEEEEEE){
        fDecodeStatus.fEventNumber = pbuffer[i]&0xFFF;
      }
      else{
        fDecodeStatus.fBufferLength = pbuffer[i]&0xFFF;
        fDecodeStatus.fStatus = E_EventHeader;
      }
      break;
    }
    case E_BufferTerminator: {
      if(pbuffer[i] != 0xFFFF || fDecodeStatus.fBufferLength !=1){
        // format error
        // TODO
      }
      else{
